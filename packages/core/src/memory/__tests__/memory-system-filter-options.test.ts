...
describe("MemorySystem recall filter options (contextId/userId/scope)", () => {
  ...
  beforeAll(async () => {
    ...
    await memory.remember("filter test note", {
      scope: "context",
      contextId: "ctx-1",
      metadata: { docId: "D-ctx1" },
    } as any);
    await memory.remember("filter test note", {
      scope: "context",
      contextId: "ctx-2",
      metadata: { docId: "D-ctx2" },
    } as any);
    await memory.remember("filter test note", {
      scope: "global",
      metadata: { docId: "D-global" },
    } as any);
  });
  ...
  it("filters by contextId", async () => {
    const res1 = await memory.recall("filter test note", {
      contextId: "ctx-1",
      include: { metadata: true },
      limit: 10,
    } as any);
    const res2 = await memory.recall("filter test note", {
      contextId: "ctx-2",
      include: { metadata: true },
      limit: 10,
    } as any);
  ...
  it("filters by scope", async () => {
    const resGlobal = await memory.recall("filter test note", {
      scope: "global",
      include: { metadata: true },
      limit: 10,
    } as any);
