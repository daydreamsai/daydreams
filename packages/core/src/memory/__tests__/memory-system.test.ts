...
describe("MemorySystem behaviors", () => {
  ...
  it("rememberBatch chunking with overlap returns multiple ids and indexes chunks", async () => {
    ...
    const allInNs = await memory.vector.search({
      query: "",
      namespace: "batch-ns",
      limit: 10,
    });
  ...
  it("recall dedupeBy docId collapses duplicates", async () => {
    ...
    await memory.vector.index([
      ...
      {
        id: "dup-2",
        content: "ipsum dolor extended text",
        metadata: { docId: "X" },
      },
      {
        id: "uniq-1",
    ...
    const withoutDedupe = await memory.recall("ipsum dolor", {
      include: { content: true, metadata: true },
      dedupeBy: "none",
    } as any);
    expect(withoutDedupe.length).toBeGreaterThanOrEqual(2);

    const withDedupe = await memory.recall("ipsum dolor", {
      include: { content: true, metadata: true },
      dedupeBy: "docId",
    } as any);
    ...
    expect(
      withDedupe.filter((r) => (r.metadata as any)?.docId === "X").length
    ).toBe(1);
  });
});
