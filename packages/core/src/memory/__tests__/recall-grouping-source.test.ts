...
describe("MemorySystem recall grouping by source", () => {
  ...
  beforeAll(async () => {
    ...
    await memory.vector.index([
      {
        id: "s1-exact",
        content: "group test content",
        metadata: { source: "s1", docId: "D1" },
      },
      {
        id: "s1-start",
        content: "group test content more",
        metadata: { source: "s1", docId: "D1" },
      },
      {
        id: "s2",
        content: "group test other source",
        metadata: { source: "s2", docId: "D2" },
      },
    ]);
  });
  ...
  it("keeps best per source", async () => {
    const res = await memory.recall("group test content", {
      groupBy: "source",
