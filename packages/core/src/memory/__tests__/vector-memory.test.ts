...
describe("VectorMemoryImpl basic operations", () => {
    ...
    await memory.vector.index([
      {
        id: "doc1",
        content: "hello world",
        metadata: { docId: "D1", tag: "alpha" },
      },
      {
        id: "doc2",
        content: "hello there world",
        metadata: { docId: "D2", tag: "beta" },
      },
      {
        id: "doc3",
        content: "namespaced alpha content",
        namespace: "nsB",
        metadata: { docId: "D3", tag: "alpha" },
      },
    ]);
  });
  ...
  it("applies namespace and metadata filters", async () => {
    const ns = await memory.vector.search({
      query: "alpha",
      namespace: "nsB",
      limit: 10,
      includeContent: true,
    ...
    const byMeta = await memory.vector.search({
      query: "hello",
      filter: { tag: "beta" },
      limit: 10,
    });
