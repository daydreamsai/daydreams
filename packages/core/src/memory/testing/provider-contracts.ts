...
export function contractTestKeyValueProvider(
  ...
  describe(`KeyValueProvider contract: ${name}`, () => {
    ...
    it("respects ifNotExists option", async () => {
      ...
      await expect(
        provider.set("unique", 2, { ifNotExists: true })
      ).rejects.toBeTruthy();
  ...
  describe(`VectorProvider contract: ${name}`, () => {
      ...
      const docs: VectorDocument[] = [
        {
          ...
          metadata: { source: "test", docId: "1" },
        },
        {
          id: "d2",
          content: "Jumps over the lazy dog",
          metadata: { source: "test", docId: "2" },
        },
        {
          id: "d3",
          content: "A different topic entirely",
          metadata: { source: "alt", docId: "3" },
        },
      ];
    ...
    it("applies namespace and metadata filters", async () => {
      await provider.index([
        ...
        {
          id: "n3",
          content: "alpha in nsB",
          namespace: "nsB",
          metadata: { tag: "alpha" },
        },
      ]);

      const byNs = await provider.search({
        query: "alpha",
        namespace: "nsA",
        limit: 10,
      });
      ...
      const byMeta = await provider.search({
        query: "beta",
        filter: { tag: "beta" },
        limit: 10,
      });
    ...
    it("updates and deletes documents", async () => {
      await provider.index([
        { id: "upd", content: "original text", metadata: { v: 1 } },
      ]);
      await provider.update("upd", {
        content: "updated text",
        metadata: { v: 2 },
      });
      const afterUpdate = await provider.search({
        query: "updated",
        limit: 5,
        includeContent: true,
        includeMetadata: true,
...
export function contractTestGraphProvider(
  ...
  describe(`GraphProvider contract: ${name}`, () => {
    ...
    it("adds, gets, updates, and deletes nodes", async () => {
      const id = await provider.addNode({
        id: "",
        type: "person",
        properties: { name: "Ada" },
      });
    ...
    it("adds edges and queries by direction", async () => {
      const a = await provider.addNode({
        id: "",
        type: "person",
        properties: { name: "A" },
      });
      const b = await provider.addNode({
        id: "",
        type: "person",
        properties: { name: "B" },
      });
      const e = await provider.addEdge({
        id: "",
        from: a,
        to: b,
        type: "knows",
