...
Firebase Firestore integration for the DaydreamsAI memory system. This package
provides persistent key-value storage using Firebase Firestore while using
in-memory providers for vector and graph operations.
...
4. **Key Design**: Use hierarchical keys (`user:123:profile`) for better
   organization
...
1. **Authentication Errors**
   ...
   - Ensure `GOOGLE_APPLICATION_CREDENTIALS` is set, or provide `serviceAccount`
     in config
...
2. **Permission Errors**
   ...
   - Ensure your service account has "Firestore Admin" or "Cloud Datastore User"
     role
...
4. **Firestore Rules**
   ...
   - For server-side usage, rules typically allow all operations for
     authenticated users
